 ...
 */
export const JH_LOGIN_WEB_BUILD = '202606211900';
...
export function createVolunteerSduiController(options) {
  ...
  function bindRootActions() {
    ...
    rootEl.addEventListener('click', (e) => {
      const el = e.target.closest('[data-sdui-action]');
      if (!el || !rootEl.contains(el)) return;
      e.preventDefault();
      e.stopPropagation();
      try {
        const action = JSON.parse(el.dataset.sduiAction);
        handleAction(action);
      } catch (err) {
        console.error('SDUI action parse', err);
      }
    });
  }
  ...
  function attachAction(el, action) {
    if (!action) return;
    el.dataset.sduiAction = JSON.stringify(action);
  ...
  function renderScreen(envelope) {
    ...
    rootEl.classList.toggle('jh-sdui-sticky-header', stickyHeader || homeSplit);
    ...
    const wrap = document.createElement('div');
    if (homeSplit) {
      wrap.className = 'jh-sdui-home-middle';
    } else {
