...
**Implemented** in `buddhist-stone-ios-app/private-server`: `GET/POST https://api.karmadots.org/jewelheart/...` (same auth as `/user/*`). SDUI schedule screens live in **`src/jewelheart/sduiScreens.js`** (`retreat.schedule`, `retreat.schedule.day`, alias `retreat.home`). Run `npm run db:jewelheart` in `private-server` after setting `DATABASE_URL`. Seed a global admin with `INSERT INTO jewelheart_admins (firebase_uid) VALUES ('your-firebase-uid');` or rely on per-retreat access: creating a retreat adds your UID to `jewelheart_retreat_admins`.
