 ...
 */
export const JH_LOGIN_WEB_BUILD = '2026-07-01-11:01';
...
export function createVolunteerSduiController(options) {
  ...
  async function handleVolunteerUserManage(action) {
    ...
    try {
      ...
      if (op === 'unlink') {
        const name = action.payload?.displayName || params.userManageVolunteerName || 'this volunteer';
        if (!window.confirm(`Unlink sign-in for ${name}? They will need a new invite to sign in again.`)) {
          return;
        }
      ...
      if (op === 'sendInvite') {
        if (action.payload?.hasEmail === false) {
          setMsg('No email on roster — add an email address first.', true);
          return;
        }
        setMsg('Sending invite…', false);
        const res = await fetch(`${base}/invite-email`, {
          method: 'POST',
          headers: { Authorization: `Bearer ${token}`, 'Content-Type': 'application/json' },
          body: '{}',
        });
        const j = await res.json().catch(() => ({}));
        if (!res.ok) throw new Error(j.error || `HTTP ${res.status}`);
        setMsg(j.message || 'Invite sent.', false);
        return load();
      }
