/**
 * Volunteer home SDUI for KarmaDots private-server (sduiScreens.js).
 * Applied by scripts/apply-jewelheart-sdui-fragment.mjs — do not paste into service.js.
 *
 * @param {{ listRetreats: Function, getScheduleByDay: Function, query: Function }} deps
 */
export function createJewelheartHomeSdui(deps) {
  const { listRetreats, getScheduleByDay, query } = deps;
  ...
  async function jewelheartHomeSdui(firebaseUid) {
    ...
    try {
      ...
      if (!retreat) {
        ...
        if (!volunteerId) {
          usingDemo = true;
        } else if (typeof getScheduleByDay !== 'function') {
          usingDemo = true;
        } else {
          const dates = volunteerHomeRetreatDates(retreat);
          const mine = [];
          for (const iso of dates) {
            const day = await getScheduleByDay(firebaseUid, retreat.id, iso);
